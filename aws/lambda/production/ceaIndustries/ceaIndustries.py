...
def lambda_handler(event, context):
    """
    This function handles mySQL CRUD for CEA data element Industries.
    Receives a CRUD instruction from the event body "lambda" parameter value.
    Returns a Industries recordset in all cases.
    ...
    if id < 0:
        ...
        if command == "select":
            sql = "CALL cea.sp_industries_get(null)"

    else:
        if command == "select":
            sql = "CALL cea.sp_industries_get('%s')" % (id)
